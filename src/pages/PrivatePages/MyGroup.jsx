...
const MyGroups = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/allgroups?email=${user.email}`)
      .then((res) => res.json())
      .then((data) => {
        setGroups(data);
        setLoading(false);
  ...
  const handleDelete = (email) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`${import.meta.env.VITE_API_URL}/allgroups/${email}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="View and manage your groups on HobbyHub" />
      </Helmet>

      <div className="overflow-x-auto my-12 px-4 mb-8">
        <h2 className="text-2xl font-bold mb-4 text-center primary">
          My Groups
        </h2>
              ...
              <table className="table table-zebra w-full bg-base-100 text-sm">
                ...
                <tbody>
                  {groups.map((group, index) => (
                    <tr key={group._id}>
                      <td>{index + 1}</td>
                      <td className="font-bold font-primary">
                        {group.groupname}
                      </td>
                      <td>{group.member}</td>
              ...
              <table className="table table-zebra w-full bg-base-100 text-sm">
                ...
                <tbody>
                  {groups.map((group, index) => (
                    <tr key={group._id}>
                      <td>{index + 1}</td>
                      <td className="font-bold font-opensans">
                        {group.groupname}
                      </td>
                      <td>{group.member}</td>
